...
- Coordinador: correo institucional + contraseña.
...
- Revisión global multi-modelo con consenso y credenciales cifradas.
...
El estudiante ingresa por `#/student` con su cédula y un PIN institucional de 6 dígitos. El PIN es emitido o restablecido por Administración y se almacena únicamente como hash PBKDF2 en servidor.
...
Registro del intento institucional
                    ↓
Revisión académica multi-modelo (capa adicional)
                    ↓
          ≤20 %             >20 %
          Cumple           No cumple
             ↓                 ↓
...
La revisión IA no puede modificar el porcentaje oficial de similitud. Si los proveedores IA fallan, el intento antiplagio ya registrado se conserva.
...
`AI_CREDENTIALS_MASTER_KEY` debe ser larga, aleatoria e independiente de `SUPABASE_SERVICE_ROLE_KEY`. Las credenciales de modelos IA se cifran y nunca se leen directamente desde el navegador.

Los endpoints personalizados de los modelos quedan restringidos por base de datos a HTTPS y a los proveedores autorizados: Google Gemini, Groq, OpenRouter, Cohere y Cloudflare.
...
El código, las migraciones y las Edge Functions deben mantenerse sincronizados. Un cambio de frontend que dependa de `phase30.sql` no queda operativo hasta aplicar la migración y desplegar las Edge Functions correspondientes.

Antes de habilitar el módulo multi-modelo, aplica `phase29.sql` y `phase30.sql`, configura `AI_CREDENTIALS_MASTER_KEY`, carga las claves de proveedor desde Administración y prueba cada modelo. No guardes claves API en GitHub.
