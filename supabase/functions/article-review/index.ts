...
import { firestoreAdminList, type FirestoreRecord } from '../_shared/firestore-admin.ts';

const RUBRIC_VERSION = 'academic-15-criteria-2026-v2';
const PROMPT_VERSION = 'firebase-parallel-all-enabled-v4';
...
interface Model { id:string; provider:string; adapter:Adapter; runtime:'cloud'|'local'; display_name:string; model_id:string; api_url:string|null; priority:number; timeout_ms:number; max_concurrency:number; }
...
interface Result { evaluator_slot:number; evaluator_name:string; source_id:string; model_ref:null; provider:string; provider_model_id:string; adapter:string; score:number|null; duration_ms:number; status:'completed'|'failed'; findings:Finding[]; error_message:string|null; started_at:string; completed_at:string; }
...
async function evaluate(slot:number,ctx:Context,text:string,pageCount:number|null):Promise<Result> {
  const start=Date.now(),started_at=new Date().toISOString(); const base={evaluator_slot:slot,evaluator_name:ctx.model.display_name,source_id:ctx.model.id,model_ref:null as null,provider:ctx.model.provider,provider_model_id:ctx.model.model_id,adapter:ctx.model.adapter,duration_ms:0,started_at,completed_at:''};
...
function model(r:FirestoreRecord):Model { return {id:str(r.id),provider:str(r.provider)||'Proveedor',adapter:(str(r.adapter)||'openai') as Adapter,runtime:str(r.runtime)==='local'?'local':'cloud',display_name:str(r.display_name)||str(r.model_id)||'IA',model_id:str(r.model_id),api_url:str(r.api_url)||null,priority:num(r.priority)??0,timeout_ms:num(r.timeout_ms)??DEFAULT_TIMEOUT,max_concurrency:Math.max(1,Math.round(num(r.max_concurrency)??2))}; }
async function contexts():Promise<Context[]> {
  const modelRows=(await firestoreAdminList(MODELS_COLLECTION)).filter(row=>bool(row.enabled)).sort((a,b)=>(num(b.priority)??0)-(num(a.priority)??0)||str(a.display_name).localeCompare(str(b.display_name)));
  if(!modelRows.length)return [];
  const credentialRows=await firestoreAdminList(CREDENTIALS_COLLECTION),credentialMap=new Map<string,FirestoreRecord>(credentialRows.map(row=>[str(row.id),row]));
  const out:Context[]=[];
  for(const raw of modelRows){const m=model(raw); let apiKey:string|null=null,error:string|null=null; try{if(m.runtime==='local')throw new Error('Requiere agente local seguro; el servidor no puede acceder al localhost del usuario.'); if(!m.model_id)throw new Error('Falta Model ID'); const c=credentialMap.get(m.id); if(!c)throw new Error('Falta API key'); apiKey=await decrypt(c); validateUrl(m.api_url||defaultUrl(m));}catch(e){error=safeError(e);} out.push({model:m,apiKey,configError:error});}
  return out;
}
...
Deno.serve(async (request:Request):Promise<Response> => {
  ...
  try{
    ...
    const all=await contexts(); if(!all.length)return json(request,{error:'No hay IA habilitadas en Firebase. Configura y activa al menos una desde Administración.'},503); const p=prompt(text,{page_count:v.data.page_count,word_count:v.data.word_count,title:d.data?.title??null,career:d.data?.career??null,modality:d.data?.modality??null},integrity),count=all.length,minSuccess=count<=1?1:Math.max(2,Math.ceil(count*MIN_SUCCESS_RATIO)),slots=Array.from({length:count},(_,i)=>i+1);
    const modelIds=all.map(x=>x.model.id).filter(id=>/^[0-9a-fA-F-]{36}$/.test(id));
    const run=await service.from('article_review_runs').insert({target_version_id:versionId,analysis_attempt_id:attemptId,requested_by:user.data.user.id,status:'running',rubric_version:RUBRIC_VERSION,prompt_version:PROMPT_VERSION,evaluator_count:count,successful_evaluators:0,evaluator_slots:slots,selected_model_ids:modelIds,successful_model_ids:[],minimum_consensus:0,minimum_success:minSuccess,config_snapshot:{source:'firebase',collection:MODELS_COLLECTION,all_enabled_models:all.map((x,i)=>({slot:i+1,id:x.model.id,name:x.model.display_name,provider:x.model.provider,model_id:x.model.model_id,configured:!x.configError})),parallelism:MAX_PARALLEL},similarity_percent:similarity}).select('*').single(); if(run.error||!run.data)throw new Error(run.error?.message||'No fue posible crear la revisión global'); runId=String(run.data.id);
    const results=await pool(all,MAX_PARALLEL,(ctx,i)=>evaluate(i+1,ctx,p,num(v.data.page_count))); const rows=results.map(r=>({run_id:runId,evaluator_slot:r.evaluator_slot,evaluator_name:r.evaluator_name,model_ref:null,provider:r.provider,provider_model_id:r.provider_model_id,adapter:r.adapter,score:r.score,duration_ms:r.duration_ms,status:r.status,findings:r.findings,error_message:r.error_message,started_at:r.started_at,completed_at:r.completed_at})); const saved=await service.from('article_reviewer_results').insert(rows); if(saved.error)throw saved.error;
    ...
    const successfulIds=ok.map(r=>r.source_id).filter(id=>/^[0-9a-fA-F-]{36}$/.test(id));
    const done=await service.from('article_review_runs').update({status,successful_evaluators:ok.length,successful_model_ids:successfulIds,final_score:finalScore,performance_level:finalScore===null?null:performance(finalScore),error_message:message,completed_at:new Date().toISOString()}).eq('id',runId).select('*').single(); if(done.error||!done.data)throw new Error(done.error?.message||'No fue posible cerrar la revisión global'); return json(request,{run:done.data,findings,reviewers:results});
