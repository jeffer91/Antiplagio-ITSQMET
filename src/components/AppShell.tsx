...
interface AppShellProps extends PropsWithChildren { role: AppRole; }

const roleLabels: Record<AppRole, string> = { student: 'Estudiante', coordinator: 'Coordinador', admin: 'Administrador' };
function canAcknowledge(notification: AppNotification): boolean { return notification.kind !== 'supplementary_required'; }
...
export function AppShell({ role, children }: AppShellProps): React.JSX.Element {
  ...
  const refreshNotifications = useCallback(async (): Promise<void> => {
    try { setNotifications(await loadNotifications()); } catch { setNotifications([]); }
  }, []);

  useEffect(() => {
    let active = true;
    const refresh = async (): Promise<void> => { if (active) await refreshNotifications(); };
    void refresh();
  ...
  const acknowledge = async (notification: AppNotification): Promise<void> => {
    ...
    try { await resolveNotification(notification.id); await refreshNotifications(); }
  ...
  return (
    <div className={`plagguard-layout role-${role}`}>
      <header className="plagguard-topbar">
        ...
        <div className="topbar-actions">
          <div className="notification-wrap">
            <button className={`notification-bell ${notifications.length > 0 ? 'has-alerts' : ''}`} type="button"
              title={`${notifications.length} alertas pendientes`} aria-label={`${notifications.length} alertas pendientes`}
              aria-expanded={notificationsOpen} onClick={() => setNotificationsOpen((current) => !current)}>
              <span aria-hidden="true">●</span>
              {notifications.length > 0 && <strong>{notifications.length}</strong>}
            </button>
            {notificationsOpen && (
              <div className="notification-popover" role="status">
                <div className="notification-popover-heading"><strong>Alertas de PlagGuard</strong><span>{notifications.length} pendientes</span></div>
                {notifications.length === 0 ? <p className="notification-empty">No tienes alertas pendientes.</p> : (
      ...
      <main className="plagguard-main">
        {important && <div className={`persistent-alert ${important.kind}`}><strong>{important.title}</strong><span>{important.message}</span></div>}
        {children}
      </main>
    </div>
  );
