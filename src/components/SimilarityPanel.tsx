...
import type { AnalysisProgress, SimilarityAnalysisResult } from '../types/similarity';
  ...
  return (
    <section className="similarity-panel">
      <div className="similarity-heading">
        ...
        {analysis && <div className={`similarity-score ${analysis.similarity_percent >= 30 ? 'high' : analysis.similarity_percent >= 15 ? 'medium' : 'low'}`}><strong>{analysis.similarity_percent.toFixed(1)}%</strong><span>similitud</span></div>}
      </div>
      ...
      {analysis && (
        <>
          <div className="similarity-stats">
            <span><strong>{analysis.matched_words.toLocaleString('es-EC')}</strong> palabras coincidentes</span>
          ...
          <div className="analysis-meta">Último análisis: {formatDate(analysis.created_at)} · {analysis.algorithm_version}</div>
          ...
          {analysis.sources.length === 0 ? (
            ...
            <div className="source-list">
              {analysis.sources.slice(0, 8).map((source, index) => (
          ...
          {canRun && (
            <div className="similarity-actions">
              <button className="secondary-button compact-button" type="button" disabled={running} onClick={() => void run()}>Analizar de nuevo</button>
