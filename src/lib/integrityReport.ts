...
export function buildIntegrityReportHtml(report: IntegrityReportRecord): string {
  ...
  <h2>Resumen ejecutivo</h2><div class="cards"><div class="card"><b>${pct(s.summary.consolidated_similarity_adjusted)}</b><small>Consolidada ajustada</small></div><div class="card"><b>${pct(s.summary.internal_similarity_adjusted)}</b><small>Institucional ajustada</small></div><div class="card"><b>${pct(s.summary.external_similarity_verified)}</b><small>Externa verificada</small></div><div class="card"><b>${s.summary.ai_evidence_score === null ? 'N/D' : `${s.summary.ai_evidence_score.toFixed(0)}/100`}</b><small>IA semántica</small></div></div><div class="notice">La similitud consolidada usa cobertura única de palabras; no suma porcentajes. La IA semántica solo valida paráfrasis contra fuentes reales localizadas.</div>
  ...
  <h2>4. Validación semántica de similitud con IA</h2>${s.ai_writing ? `<p>Índice <b>${s.ai_writing.evidence_score.toFixed(0)}/100</b> · ${s.ai_writing.flagged_word_percent.toFixed(1)} % de palabras señaladas.</p>${ai || '<p>Sin coincidencias semánticas adicionales validadas por IA.</p>'}` : '<p>No existe validación semántica con IA.</p>'}
  <h2>5. Trazabilidad</h2><p class="hash"><b>Huella informe:</b> ${htmlEsc(report.snapshot_sha256)}<br><b>Esquema:</b> ${htmlEsc(report.report_schema_version)}<br><b>Interno:</b> ${htmlEsc(s.provenance.internal_analysis_id ?? 'N/D')}<br><b>Externo:</b> ${htmlEsc(s.provenance.external_analysis_id ?? 'N/D')}<br><b>Citas:</b> ${htmlEsc(s.provenance.citation_analysis_id ?? 'N/D')}<br><b>IA:</b> ${htmlEsc(s.provenance.ai_analysis_id ?? 'N/D')}</p><div class="footer">PlagGuard presenta evidencia técnica de similitud trazable. La IA semántica solo cuenta cuando la coincidencia está vinculada a una fuente verificable y cumple las reglas institucionales de cobertura.</div></body></html>`;
}
...
export function buildIntegrityReportSpreadsheet(report: IntegrityReportRecord): string {
  ...
  const summary = [
    ...
    xlsRow(['Evidencia semántica IA', s.summary.ai_evidence_score ?? 'N/D']),
    xlsRow(['% palabras con coincidencia semántica IA', s.summary.ai_flagged_word_percent ?? 'N/D']), xlsRow(['Huella informe', report.snapshot_sha256]),
  ];
